...
public class TopDefender extends Player {
	public TopDefender(int x, int y, double angle, Color c, int s){
		super(x,y,angle,c,s);
	}
	public TopDefender(Color c, int s){
		super(c,s);
		playerSpeed = SPEED+0.2;
	}
	@Override
	public void goToInitalPosition(int width, int height) {
		centerX = MARGIN+(width-MARGIN*2)/5 + side*((width-40)*0.6);
		centerY = MARGIN+(height-MARGIN*2)/4;
	}
