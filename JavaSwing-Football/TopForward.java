...
public class TopForward extends Player {
	public TopForward(int x, int y, double angle, Color c, int s){
		super(x,y,angle,c,s);
	}
	public TopForward(Color c, int s){
		super(c,s);
		playerSpeed = SPEED;
	}
	@Override
	public void goToInitalPosition(int width, int height) {
		centerX = MARGIN+(width-MARGIN*2)*0.4+side*(width-MARGIN*2)/5;
		centerY = MARGIN+(height-40)/4;
	}
